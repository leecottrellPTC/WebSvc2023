...
public class BookController {
    ...
    public ResponseEntity<String> postNewBook(@RequestBody Book newBook) {
        // Book newBook = new Book(null, null, null, null, null, null)
        String response = newBook.getTitle() + " added to database";
        String SQL = "";
        Connection con;
        try {
            ...
            SQL = String.format("insert into book values('%s', '%s', '%s', '%s', '%s', '%s')", newBook.getBookCode(),
                    newBook.getTitle(), newBook.getPublisherCode(), newBook.getType(),
                    newBook.getPaperback(), newBook.getImage());
            
            if (stmt.execute(SQL)) {
                response = newBook.getTitle() + " not added " + SQL;
    ...
    public ResponseEntity<String> updateBook(@RequestBody PutClass data) {
        // Book newBook = new Book(null, null, null, null, null, null)

        String response = data.getBookCode() + " updated database";
        String SQL = "";
        Connection con;
        try {
            con = DriverManager.getConnection(connectionString);
            ...
            SQL = String.format("update book set %s = '%s' where bookCode = '%s'",
                data.getField(), data.getValue(), data.getBookCode());
                stmt.execute(SQL);
        } catch (Exception e) {
            // TODO Auto-generated catch block
            response = e.getMessage() + " -- " + SQL;
        ...
        return new ResponseEntity<String>(response + " " + SQL, HttpStatus.OK);
    }
